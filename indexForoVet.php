...
<h1>Foro de atención veterinaria</h1>
<table class="tabla1" width="80%" >
	

<?php 
	include("modelo/conexForo.php");
	$query = "SELECT * FROM  forov WHERE identificador = 0 ORDER BY fecha DESC";
	...
	<td class='dos' width='200px'><b>Respuestas</b></td>";
if($cadena==2){
...
echo "</tr>";
	...
	while($row = mysqli_fetch_array($result, MYSQLI_ASSOC)){
		...
		echo "<tr>";
			echo "<td class='dos'><a class='ver' href='vista/vistaForoVet.php?id=$id&id2=$id3'><b>ver</b></a></td>";
	...
	if($cadena==1 || $cadena==2){
		$id2=$_GET['id'];
		$query2 = "SELECT rol  FROM usuarioespe WHERE codigoUsu =" . $id2;
		if($query2 = $mysqli->query($query2)){
			$fila = $query2->fetch_assoc();
			
		
				echo"</table>
				<form class='form' action='formularioNForoVet.php'>
