...
<a name="Inicio"></a>
    ...
    <table class="table caption-top">
            ...
            <tr>
                <th>Autor</th><th>Título</th><th>Mensaje</th><th>Fecha</th><th>Respuestas</th><th>ID</th><th>Eliminar</th>
            </tr>
<?php
        require_once '../modelo/conexAdop.php';
        $foro=$pdo->query("SELECT *  FROM foro ");
        $enlace2 ="<a href='../controlador/ctrlCliente.php?accion=eliminar3&id=";
        
        ?>        
                        <?php
                            while ($perFila=$foro->fetch(PDO::FETCH_ASSOC)){
                                if($perFila['identificador']==0){
                                    echo "<tr><td>". $perFila['autor'] ."</td><td>". $perFila['titulo'] ."</td><td>". $perFila['mensaje'] ."</td><td>". $perFila['fecha'] ."</td><td>". $perFila['respuestas'] ."</td><td>". $perFila['identificador'] ."</td><td>" .$enlace2 .$perFila['identificador'] ."'><i class='fas fa-trash-alt'></i></a></td></tr>";
