...
<a name="Inicio"></a>
    ...
    <table class="table caption-top">
            <caption>Listado Animales en adopcion</caption>
            <tr>
                <th  >Foto</th><th>ID</th><th>Especie</th><th>Raza</th><th>Edad</th><th>Sexo</th><th>Peso</th><th>Control</th><th>Proveniencia</th><th>Eliminar</th>
            </tr>
<?php
        require_once '../modelo/conexAdop.php';
        $tarjetas=$pdo->query("SELECT *  FROM daradop ");
        $enlace2 ="<a href='../controlador/ctrlCliente.php?accion=eliminar2&id=";
        ?>        
                        <?php
                            while ($perFila=$tarjetas->fetch(PDO::FETCH_ASSOC))
                            {
                                echo "<tr><td>". "<img width='100' height='100' class='targetIMG' src='data:image/png;base64," .base64_encode($perFila['Foto'])." '  >"  ."</td><td>". $perFila['idDarAdop'] ."</td><td>". $perFila['especie'] ."</td><td>". $perFila['raza'] ."</td><td>". $perFila['Edad'] ."</td><td>". $perFila['Sexo'] ."</td><td>". $perFila['Peso'] ."</td><td>". $perFila['Control'] ."</td><td>". $perFila['Proveniencia'] ."</td><td>" .$enlace2 .$perFila['idDarAdop'] ."'><i class='fas fa-trash-alt'></i></a></td></tr>";
                            }
?>
